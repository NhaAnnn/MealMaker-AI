...
try:
    # 1. Đọc chuỗi JSON từ Biến Môi Trường (Ưu tiên)
    ...
    # db vẫn là None nếu lỗi, các API phụ thuộc vào DB sẽ trả về 500
...
def get_auto_tags():
    ...
    try:
        ...
        final_tags = [tag for tag in final_tags if tag not in TIME_TAGS] # Loại bỏ tag AI dự đoán
        ...
        if time_tag_rule:
            final_tags.append(time_tag_rule) # Thêm tag thời gian chính xác
...
def get_recommendations_ai():
    # Kiểm tra kết nối DB trước tiên
    ...
    if not recommendation_ids:
        ...
        try:
            ...
            if not user_doc.exists:
                print("User chưa tồn tại trong DB. Bỏ qua Level 2.")
                pass
            else:
...
@app.route("/")
def hello():
    return "Chào! Server AI (Python Hybrid Microservice v3 - Fully Patched) đang chạy!"
...
if __name__ == "__main__":
    # Sử dụng port mặc định hoặc từ biến môi trường nếu có
